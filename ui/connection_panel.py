...
class ConnectionPanel(QGroupBox):
    ...
    def __init__(self, parent=None):
        ...
        self.channel_value_label.setObjectName(
            "channelValue"
        )
        ...
        layout.addWidget(
            self.channel_value_label
        )

        layout.addSpacing(12)

        layout.addWidget(
            QLabel("Mevcut CAN Baud Rate:")
        )
    ...
    def get_selected_baud_rate_kbit(self):
        return int(
            self.baud_rate_combo.currentData()
        )

    def get_selected_baud_rate(self):
        return (
            self.get_selected_baud_rate_kbit()
            * 1000
        )

    def set_baud_rate(self, baud_rate_kbit):
        index = self.baud_rate_combo.findData(
            baud_rate_kbit
        )
        ...
        self.baud_rate_combo.setCurrentIndex(
            index
        )

    def set_channel(self, channel):
        self.channel_value_label.setText(
            str(channel)
        )
